...
def copy_and_modify_layers(source_model_path, target_model_path, output_model_path):
    ...
    #   #### 新torch
    if isinstance(source_model, dict) and 'ema' in source_model:
        source_model = source_model['ema']
    ...
    #   #### 新torch
    # if isinstance(target_model, dict) and 'ema' in target_model:
    #     target_model = target_model['ema']
...
# 因为我的backbone和MCF一样 前30行不变 所以前面的也不用改
copy_and_modify_layers(
    source_model_path="runs\M3FD\M3FD-yolo11n-RGB\weights/best.pt",    # input: step 1
    target_model_path="runs\M3FD\M3FD-yolo11n-ECFFN-step2/weights/best.pt", # input: step 2
    output_model_path='runs\M3FD\M3FD-yolo11n-ECFFN-step2/M3FD-yolo11n-ECFFN-step3.pt'  # output: step 3

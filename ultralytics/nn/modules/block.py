...
class RIFusion1(nn.Module):
    ...
    def __init__(self, c1, r=16, dimension=1):
        super().__init__()
        # self.c1=c1*2
        self.c1 = 1024 if c1 == 1024 else c1 * 2
        # 这行代码实现全局平均池化功能，其作用是将输入特征图的每个通道的空间维度（H×W）压缩为一个标量值（1×1），从而生成通道级别的统计量（通道描述符）。
        # 若输入特征维度为 C×H×W（C为通道数，H和W为高和宽），经过该操作后输出维度为 C×1×1。
        # 此时每个通道的描述符仅反映该通道自身的全局空间信息（如红外或可见光通道的均值），而不同通道之间（例如拼接后的RGB通道与IR通道）并未进行任何交互，通道描述符的生成过程完全独立。
        self.avg_pool = nn.AdaptiveAvgPool2d(1)
